...
> Note: fastmigrate runs ``.sql`` migrations using Python's built-in sqlite3.
> You only need the external ``sqlite3`` binary if **your own** ``.sh`` migrations
> invoke it, or if you want to inspect the database from the command line.
